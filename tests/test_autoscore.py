...
def test_autoscore_initialization(mock_listdir, mock_read_csv, mock_data):
    ...
    assert 'score' not in autoscore.data.columns, "'score' column should not exist initially"
...
def test_rank_cars(mock_listdir, mock_read_csv, mock_data):
    ...
    assert ranked_cars["score"].iloc[0] >= ranked_cars["score"].iloc[1], "Cars should be ranked by score in descending order"
...
def test_rank_unique_cars(mock_listdir, mock_read_csv, mock_data):
    ...
    assert len(ranked_cars.drop_duplicates(subset=["make", "model"])) == 3, "There should be 3 unique cars based on make-model"
...
def test_normalize_with_empty_data(mock_listdir, mock_read_csv, mock_data):
    ...
    assert normalized_value == 1, "Normalization should return 1 when min == max (edge case)"
